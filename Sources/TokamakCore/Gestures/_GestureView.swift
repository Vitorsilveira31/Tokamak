...
public struct _GestureView<Content: View, G: Gesture>: _PrimitiveView {
    final class Coordinator<GE: Gesture>: ObservableObject {
        var gesture: GE
    ...
    public func onPhaseChange(_ phase: _GesturePhase) {
        ...
        switch phase {
        case .began(let context) where context.eventId != nil:
            startDelay()
...
extension View {
    ...
    @ViewBuilder
    public func gesture<T>(_ gesture: T?, including mask: GestureMask = .all) -> some View
    where T: Gesture {
        if let gesture {
    ...
    /// - Returns: A modified version of the view with the gesture attached.
    @ViewBuilder
    public func simultaneousGesture<T>(_ gesture: T?, including mask: GestureMask = .all)
        -> some View
    where T: Gesture {
        if let gesture {
            _GestureView(
    ...
    /// - Returns: A modified version of the view with the gesture attached.
    @ViewBuilder
    public func highPriorityGesture<T>(_ gesture: T?, including mask: GestureMask = .all)
        -> some View
    where T: Gesture {
        if let gesture {
            _GestureView(
