    ...
    public func sizeThatFits(
            ...
            for (columnIndex, column) in row.columns.enumerated() {
                if row.columns.indices.contains(columnIndex + 1) {
                    if let overrideSpacing = horizontalSpacing {
                        cache.horizontalSpacing[spannedColumns + (column.span) - 1] =
                            overrideSpacing
                    } else {
        ...
        for row in cache.rows {
            ...
            for column in row.columns {
                ...
                let size = column.subview.dimensions(
                    in: .init(
                ...
                for columnIndex in spannedColumns..<(spannedColumns + column.span) {
                    if (size.width / CGFloat(column.span))
                        > cache.columnWidths[columnIndex, default: .zero]
                    {
                    ...
                    in: .init(
                        width: unsizedAxes.contains(.horizontal)
                            ? (spannedColumns..<(spannedColumns + column.span))
                                .reduce(into: .zero) {
                                    $0 += cache.columnWidths[$1, default: .zero]
                                }
    ...
    public func placeSubviews(
        ...
        for (rowIndex, row) in cache.rows.enumerated() {
            ...
            for column in row.columns {
                ...
                let anchor =
                    column.subview[GridCellAnchor.self]
                    ?? UnitPoint(
                        x: dimensions.width == 0
                            ? 0
                            : dimensions[
                                cache.columnAlignments[
                                    spannedColumns, default: alignment.horizontal]
                            ] / dimensions.width,
                        y: dimensions.height == 0
                            ? 0
                            : dimensions[row.id?.alignment ?? alignment.vertical]
                                / dimensions.height
                    )
...
extension Grid where Content == EmptyView {
    public init(
        alignment: Alignment = .center,
        horizontalSpacing: CGFloat? = nil,
        verticalSpacing: CGFloat? = nil
