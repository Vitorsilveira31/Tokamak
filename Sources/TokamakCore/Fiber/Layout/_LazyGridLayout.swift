...
extension _LazyGridLayout {
    ...
    public func sizeThatFits(
        ...
        for (index, item) in items.enumerated() {
            ...
            if case .fixed(let fixed) = item.size {
        ...
        for item in items {
            ...
            case .adaptive(let minimum, let maximum):
                ...
                let fitSize = min(
                    max(
                        (divviedSpace - ((item.spacing ?? 8) * CGFloat(fitCount - 1)))
                            / CGFloat(fitCount),
                        minimum
                    ),
                    maximum
    ...
    public func placeSubviews(
        ...
        let contentSize = cache.resolvedItems.enumerated().reduce(into: .zero) {
            $0 +=
                $1.element
                .size
                + ($1.offset == cache.resolvedItems.count - 1 ? 0 : $1.element.item.spacing ?? 8)
        }
        ...
        let startOffset =
            ...
            - contentAlignmentID.defaultValue(
                in: .init(
                    size: .init(width: contentSize, height: contentSize), alignmentGuides: [:])
            )
        ...
        for (index, subview) in subviews.enumerated() {
            ...
            if itemIndex == cache.resolvedItems.count - 1 {
                offset[keyPath: mainAxis] +=
                    cache.mainAxisSizes[mainAxisIndex] + (spacing ?? mainAxisSpacing)
            }
        }
    }
}
